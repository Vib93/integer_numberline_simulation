...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.numberline();
    this.zerobox = this.add
      .sprite(config.width / 2, config.height / 2 + 150, "zerobox")
      .setScale(0.8);
    this.zerobox.setInteractive({
      useHandCursor: true,
      draggable: true,
      pixelPerfect: true,
    });
      ...
      function (pointer) {
        ...
        for (var i = 0; i < numberbox.length; i++) {
          if (
            ...
            numberbox[i].setTexture("numberbox");
          }
        }
      },
      this
        ...
        for (var i = 0; i < 8; i++) {
          if (
            ...
            numberbox[i]
              ...
              .setName("filled")
              .setOrigin(0.5, 0.4);
            this.zerobox.disableInteractive();
            this.zerobox.destroy();
            for (var j = 0; j < i; j++) {
  ...
  numberline() {
    ...
    var startLine = new Phaser.Geom.Line(
      config.width / 2 - 180,
      config.height / 2 - 50,
      config.width / 2 + 175,
      config.height / 2 - 50
